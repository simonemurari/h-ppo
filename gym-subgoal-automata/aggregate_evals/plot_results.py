...
def get_model_type(full_name):
    """Extract the model type from a full model filename.
    
    Examples:
        ppo_DeliverCoffee -> ppo
        h_ppo_symloss_theta_DeliverCoffee_theta_0.25 -> h_ppo_symloss_theta_0.25
        h_ppo_symloss_eps_PatrolAB_v2 -> h_ppo_symloss_eps
    """
    name_lower = full_name.lower()
    ...
    return full_name  # Return as-is if unknown
...
def _generate_success_rate_tables(base_dir, global_success_data):
    ...
    per_model_dir = os.path.join(tables_dir, "per_model")
    ...
    for eval_type, maps_data in global_success_data.items():
        ...
        for map_name, models_data in maps_data.items():
            ...
            for full_model_name, success_rate in models_data.items():
                model_type = get_model_type(full_model_name)
                typed_success_data[eval_type][map_name][model_type] = success_rate
    
    # Get all unique model types across all eval types and maps
    all_model_types = set()
    all_maps = set()
    for eval_type, maps_data in typed_success_data.items():
        for map_name, models_data in maps_data.items():
            all_maps.add(map_name)
            all_model_types.update(models_data.keys())
    
    # Sort models by order key (using model type for ordering)
    ...
    _generate_per_model_tables(per_model_dir, typed_success_data, sorted_model_types, sorted_maps)
...
def _generate_per_model_tables(per_model_dir, typed_success_data, sorted_model_types, sorted_maps):
    """Generate tables for each model type showing success rate across maps and eval types."""
    eval_order = ['standard', 'random_rules', 'h_ppo_eps_0.25', 'h_ppo_eps_0.5', 'h_ppo_eps_1.0']
