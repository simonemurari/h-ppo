...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, envs=None, action=None, apply_heuristic=False, epsilon=None):
        logits = self.actor(x)
        probs = Categorical(logits=logits)
        
        if apply_heuristic is True and envs is not None:
...
if __name__ == "__main__":
    ...
    for iteration in trange(1, args.num_iterations + 1, colour="green"):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(next_obs, envs=envs, apply_heuristic=True, epsilon=epsilon)
    ...
    if args.save_model:
        os.makedirs(f"models/h_ppo_product_{args.task}{args.run_code}", exist_ok=True)
        model_path = f"models/h_ppo_product_{args.task}{args.run_code}/h_ppo_product_seed={args.seed}.pt"
