...
def evaluate(
    ...
    # Write to CSV
    os.makedirs(f'eval_results/{group_name}', exist_ok=True)
    csv_filename = f"eval_results/{group_name}/seed={seed}_omr={overall_match_rate:.2f}.csv"
    ...
    # Write to CSV
    csv_filename = f"eval_results/{group_name}/seed={seed}_STEPACTIONS.csv"
    with open(csv_filename, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
