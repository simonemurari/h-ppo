# h_ppo_sym_kd.py — Combination of SymLoss and KD Loss
#
# SymLoss (from h_ppo_symloss.py):
#   PPO-clipped policy gradient toward the symbolic policy, weighted by advantages.
#   Active on ALL states — provides a consistent, advantage-weighted push
#   toward the symbolic policy throughout training.
#
# KD Loss (from h_ppo_kdloss.py):
#   Cross-entropy (soft distillation) toward the symbolic policy, but ONLY on
#   states where (a) a symbolic rule fires AND (b) the advantage is negative.
#   This is a targeted correction: it penalises the agent specifically when it
#   deviates from the symbolic policy in states where the agent was already doing
#   the wrong thing (negative advantage).
#
# Together they cover complementary regimes:
#   - SymLoss: nudges the whole policy distribution toward the symbolic policy
#     proportionally to how much the action mattered (advantage weighting)
#   - KD Loss: hard correction when the agent ignores a rule AND performs badly
#
# Net effect: the agent follows the symbolic policy broadly (SymLoss) and gets a
# stronger shove toward it whenever it breaks a rule in a state that mattered (KD).
#
import math
import os
import random
...
class Agent(nn.Module):
    """PPO actor-critic with both SymLoss and KD Loss support."""
    ...
    def get_symbolic_probs_and_logprob(self, x, action):
        """Returns:
          sym_probs  (B, A)  — full symbolic distribution (for KD loss)
          sym_logprob (B,)   — log π_sym(action) (for SymLoss ratio)
          has_rule    (B,)   — bool mask: True where any rule fires
        """
        suggested_actions_batch = apply_rules_batch(get_observables(x[:, 4:]))
        sym_probs = torch.ones(
            (x.shape[0], self.num_actions), device=x.device
        ) * (1.0 - self.conf_level)
        ...
        sym_logprob = Categorical(probs=sym_probs).log_prob(action)
        return sym_probs, sym_logprob, has_rule
...
if __name__ == "__main__":
    ...
    if args.track:
        ...
        wandb.init(
            ...
            monitor_gym=True,
            save_code=True,
            group=f"h_ppo_sym_kd_{args.group_name}"
        )
    ...
    for iteration in trange(1, args.num_iterations + 1, colour="green"):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                # Compute both sym_probs (for KD) and sym_logprob (for SymLoss) together
                sym_probs, logprob_symbolic, has_rule = agent.get_symbolic_probs_and_logprob(next_obs, action)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # SymLoss ratio: compare new neural policy vs stored symbolic log prob
                ...
                # ── SymLoss: advantage-weighted PPO ratio toward symbolic policy
                #    (from h_ppo_symloss — applies on ALL states, uses advantages)
                pg_loss1_sym = -mb_advantages * ratio_symbolic
                pg_loss2_sym = -mb_advantages * torch.clamp(ratio_symbolic, 1 - args.clip_coef, 1 + args.clip_coef)
                pg_loss_sym  = torch.max(pg_loss1_sym, pg_loss2_sym).mean()

                # ── KD Loss: cross-entropy toward symbolic policy, but ONLY where
                #    a rule fires AND the advantage was negative (bad state + rule existed)
                #    (from h_ppo_kdloss — targeted correction, unweighted by advantage)
                kd_loss = torch.tensor(0.0, device=device)
                active_mask = mb_has_rule & (mb_advantages_raw < 0)
                if active_mask.any():
                ...
                # Total loss:
                #   PPO RL loss
                #   + theta * SymLoss  (advantage-weighted, all states)
                #   + KD Loss          (cross-entropy, only rule+negative-advantage states)
                #   args.theta controls SymLoss weight (default 0.75, best at 1.0)
                loss = (
                    pg_loss
                    - args.ent_coef * entropy_loss
                    + args.vf_coef * v_loss
                    + args.theta * pg_loss_sym
                    + kd_loss
                )
        ...
        writer.add_scalar("losses/sym_loss",          pg_loss_sym.item(),     global_step)
    ...
    if args.save_model:
        os.makedirs(f"models/h_ppo_sym_kd_{args.size_env}x{args.size_env}_{args.n_keys}keys{args.run_code}", exist_ok=True)
        model_path = f"models/h_ppo_sym_kd_{args.size_env}x{args.size_env}_{args.n_keys}keys{args.run_code}/h_ppo_sym_kd_seed={args.seed}.pt"
        ...
        try:
            ...
            episodic_returns = evaluate(
                ...
                seed=args.seed,
                group_name=f"h_ppo_sym_kd_{args.group_name}_evals",
                Model=Agent,
                device=device,
                track=False,
                from_config=False,
